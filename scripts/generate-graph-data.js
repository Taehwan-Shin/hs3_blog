...
function generateNodeId(filePath, collectionType) {
  ...
  // Clean up the ID: lowercase, replace spaces/special chars with hyphens, but keep slashes, underscores and Unicode characters
  id = id.toLowerCase().replace(/[^\p{L}\p{N}\-\/_]/gu, "-");
...
/**
 * Read and parse markdown files from content directory recursively
 */
function readContentFiles(dirPath, baseDir = dirPath) {
  let posts = [];
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        // First check for index.md in this directory
        const indexPath = join(itemPath, "index.md");
        if (existsSync(indexPath)) {
          const content = readFileSync(indexPath, "utf-8");
          const rawSlug = itemPath.replace(baseDir, "").replace(/^\//, "").replace(/\/index\.md$/, "");
          const slug = generateNodeId(rawSlug, "posts");
        ...
        posts = posts.concat(readContentFiles(itemPath, baseDir));
      } else if (item.endsWith(".md") && item !== "index.md") {
        // Handle single-file posts
        const content = readFileSync(itemPath, "utf-8");
        // Generate slug based on relative path from baseDir
        const relativePath = itemPath.replace(baseDir, "").replace(/^\//, "");
        const rawSlug = relativePath.replace(".md", "");
        const slug = generateNodeId(rawSlug, "posts");
    ...
    log.error(`Error reading content directory ${dirPath}:`, error);
  }
...
async function generateGraphData() {
  ...
  try {
    ...
    // Phase 1: Create all post nodes first
    ...
    // Phase 2: Process connections once all nodes exist
    for (const post of visiblePosts) {
      const sourceNode = nodes.find(n => n.id === post.id);
      ...
      for (const link of allLinks) {
        // Find target post
        let targetPost = visiblePosts.find((p) => p.id === link.slug);
        
        // If not found, try matching by filename (for Obsidian-style wikilinks)
        if (!targetPost) {
          const linkFilename = link.link.split(/[-\/]/).pop().toLowerCase();
          targetPost = visiblePosts.find((p) => {
            const postFilename = p.id.split(/[-\/]/).pop().toLowerCase();
            return postFilename === linkFilename || p.id.endsWith("/" + linkFilename) || p.id === linkFilename;
          });
        }
        ...
        if (targetPost && targetPost.id !== post.id) {
          ...
          const exists = connections.some(c => 
            (c.source === post.id && c.target === targetPost.id) ||
            (c.source === targetPost.id && c.target === post.id)
          );
